...
#define PB_RETURN_ERROR(field,ret,msg) \
    do {\
...
static inline int  pb_decode_varint(pb_field_t *f)
{
    ...
    {
        if (bitpos >= 64)
            PB_RETURN_ERROR(f, -1, "varint overflow");
...
static inline int  pb_add_tag(pb_buff_t *b, pb_wire_type_t wiretype, uint32_t field_number)
{
...
{
    DBGOUT("add_vint:");
    int ret = pb_add_tag(b, PB_WT_VARINT, tag);
    if(ret)
        return -1;
...
{
    DBGOUT("add_string:");
    int ret = pb_add_tag(b, PB_WT_STRING, tag);
    if(ret)
        return -1;
...
{
    DBGOUT("add_int32:");
    int ret = pb_add_tag(b, PB_WT_FIXED32, tag);
    if(ret)
        return -1;
...
{
    DBGOUT("add_int64:");
    int ret = pb_add_tag(b, PB_WT_FIXED64, tag);
    if(ret)
        return -1;
